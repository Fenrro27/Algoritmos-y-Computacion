...
public class PeligroAlineado_Condicion implements ICondicion {
    ...
    public boolean seCumple(IMundo m) {
        ...
        if (dist <= RADIO_PELIGRO) {
            if (Math.abs(relY) < 1.0) {
                if (relX > 0 && mundo.miOrientacion.x > 0.9) return true;
                if (relX < 0 && mundo.miOrientacion.x < -0.9) return true;
            }
            if (Math.abs(relX) < 1.0) {
                if (relY > 0 && mundo.miOrientacion.y > 0.9) return true;
                if (relY < 0 && mundo.miOrientacion.y < -0.9) return true;
            }
        }
        return false;
