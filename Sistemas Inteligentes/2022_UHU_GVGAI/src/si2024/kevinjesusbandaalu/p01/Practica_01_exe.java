	...
	public static void main(String[] args) {
		...
		for (int k = 0; k < repesGlobales; k++) {
			...
			for (int lev = 0; lev < 5; lev++) {
				...
				for (int i = 0; i < 100; i++) {
					String p1 = "si2024.kevinjesusbandaalu.p01.Jugador1";
		...
		for (int i = 0; i < 5; i++) {
			System.out.println("   " + i + " ---- " + scoreRate[i] / repesGlobales + "% ------- "
					+ Math.round(scoreMean[i] / (100 * repesGlobales)*100.0)/100.0 + " ----- " + scoreMax[i]);

		}
		System.out.println("Perdidas por inacciones: " + cont2000 / repesGlobales);
		System.exit(0);
